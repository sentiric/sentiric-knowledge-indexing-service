...
class IndexingManager:
    ...
    async def initialize(self):
        try:
            ...
            logger.info("All dependencies (Vector DB, Postgres, Model) are ready.", event_name="DEPENDENCIES_READY")
    ...
    async def _wait_for_service(self, service_name: str, check_function, retries=10, delay=5):
        for i in range(retries):
            try:
                ...
                logger.info(f"{service_name} connection established.", event_name="SERVICE_CONNECTED", service=service_name)
                return
            except Exception as e:
                logger.warn(f"{service_name} connection attempt {i+1}/{retries} failed.", event_name="SERVICE_CONNECT_RETRY", service=service_name, error=str(e))
                if i == retries - 1:
                    raise e
    ...
    async def _check_qdrant(self):
        ...
        self.qdrant_client = await asyncio.wait_for(asyncio.to_thread(_sync_check), timeout=15)

    async def _check_postgres(self):
        conn = None
        try:
            conn = await asyncio.wait_for(asyncpg.connect(dsn=settings.POSTGRES_URL), timeout=10)
    ...
    async def _get_datasources_to_index(self, tenant_id: str = None) -> list[DataSource]:
        ...
        conn = None
        try:
            conn = await asyncio.wait_for(asyncpg.connect(dsn=settings.POSTGRES_URL), timeout=15)
            ...
            if datasources:
                logger.info(f"{len(datasources)} datasources queued for indexing.", event_name="DATASOURCES_QUEUED", count=len(datasources))
        except Exception as e:
            logger.error("Failed to fetch datasources.", event_name="DB_FETCH_ERROR", exc_info=True)
        finally:
            if conn:
                await conn.close()
        return datasources
    ...
    async def _update_datasource_status(self, source_id: int, status: str, update_time: bool = False):
        ...
        try:
            ...
            logger.error(f"Failed to update datasource status for {source_id}", event_name="DB_UPDATE_ERROR", source_id=source_id, error=str(e))
        finally:
            if conn:
                await conn.close()
    ...
    async def run_indexing_cycle(self, tenant_id: str = None):
        ...
        logger.info(f"Indexing cycle started for tenant: {tenant_id or 'all'}", event_name="INDEXING_CYCLE_START")
        
        try:
            datasources = await self._get_datasources_to_index(tenant_id)
            if not datasources:
                logger.info("No active datasources found.", event_name="INDEXING_CYCLE_EMPTY")
                return
            ...
            logger.info("Indexing cycle completed successfully.", event_name="INDEXING_CYCLE_END")
    ...
    async def _process_single_datasource(self, source: DataSource):
        ...
        try:
            ...
            log.info(f"Vectorizing {len(all_chunks)} chunks...", event_name="VECTORIZATION_START", chunk_count=len(all_chunks))
            ...
            def _sync_delete():
                self.qdrant_client.delete(
                    ...
                    points_selector=models.Filter(
                        must=[
                            models.FieldCondition(key="source_uri", match=models.MatchValue(value=source.source_uri))
                        ]
                    )
                )
            ...
            for i in range(0, len(points), UPSERT_BATCH_SIZE):
                ...
                def _sync_upsert(b):
                    self.qdrant_client.upsert(collection_name=collection_name, points=b, wait=True)
                
                await asyncio.wait_for(asyncio.to_thread(_sync_upsert, batch), timeout=30)
            ...
            log.info(f"Indexing successful for {total_upserted} vectors.", event_name="DATASOURCE_PROCESS_END", vectors_count=total_upserted)

        except asyncio.TimeoutError:
            log.error("Network timeout during indexing operation.", event_name="DATASOURCE_PROCESS_TIMEOUT")
            await self._update_datasource_status(source.id, "failed")
        except Exception as e:
            log.error(f"Error processing datasource: {e}", event_name="DATASOURCE_PROCESS_ERROR", exc_info=True)
            await self._update_datasource_status(source.id, "failed")
    ...
    async def ensure_collection_exists(self, collection_name: str):
        def _sync_ensure():
            try:
                ...
                logger.info(f"Creating vector collection: {collection_name}", event_name="QDRANT_COLLECTION_CREATE")
                ...
                self.qdrant_client.create_payload_index(collection_name=collection_name, field_name="source_uri", field_schema=models.PayloadSchemaType.KEYWORD)
                self.qdrant_client.create_payload_index(collection_name=collection_name, field_name="source_type", field_schema=models.PayloadSchemaType.KEYWORD)
    ...
    async def start_worker_loop(self):
        try:
            ...
            logger.error("Worker failed to initialize, entering retry loop.", event_name="WORKER_INIT_FAILED")
